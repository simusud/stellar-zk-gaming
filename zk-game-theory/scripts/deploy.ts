...
for (const contract of contracts) {
  ...
  try {
    ...
    let initArgs: string[] = ["--admin", adminAddress, "--game-hub", mockGameHubId];

    if (contract.packageName === "zk-game-theory") {
      const vkPath = join(repoRoot, "circuits", "target", "vk.hex");
      if (existsSync(vkPath)) {
        const vkHex = readFileSync(vkPath, "utf8").trim();
        console.log(`  Found ZK VK (length: ${vkHex.length})`);
        initArgs.push("--vk", vkHex);
      } else {
        console.warn("  ⚠️  ZK VK not found at circuits/target/vk.hex");
      }
      initArgs.push("--native_token", "CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC");
      initArgs.push("--treasury", adminAddress);
    }

    const deployResult =
      await $`stellar contract deploy --wasm-hash ${wasmHash} --source-account ${adminSecret} --network ${NETWORK} -- ${initArgs}`.text();
