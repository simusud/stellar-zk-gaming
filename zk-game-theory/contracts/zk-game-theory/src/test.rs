...
// Unit tests for the zk-game-theory contract using a simple mock GameHub.
...
use crate::{Error, ZkGameTheoryContract, ZkGameTheoryContractClient};
...
fn setup_test() -> (
    Env,
    ZkGameTheoryContractClient<'static>,
    ...
    // Deploy zk-game-theory with admin and GameHub address
    let contract_id = env.register(ZkGameTheoryContract, (&admin, &hub_addr));
    let client = ZkGameTheoryContractClient::new(&env, &contract_id);

    // Register zk-game-theory as a whitelisted game (mock does nothing)
...
fn test_upgrade_function_exists() {
    ...
    // Deploy zk-game-theory with admin
    let contract_id = env.register(ZkGameTheoryContract, (&admin, &hub_addr));
    let client = ZkGameTheoryContractClient::new(&env, &contract_id);
