import { Client as ZkGameTheoryClient, type Game } from './bindings/src';
...
/**
 * Service for interacting with the ZkGameTheory game contract
 */
export class ZkGameTheoryService {
  private baseClient: ZkGameTheoryClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new ZkGameTheoryClient({
  ...
  private createSigningClient(
    ...
    return new ZkGameTheoryClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new ZkGameTheoryClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
  ...
  /**
   * Commit a move (Step 1 of ZK flow)
   * Calculates commitment on-client and submits to contract
   */
  async commitMove(
    sessionId: number,
    ...
    console.log('[commitMove] Calculating commitment...');
    const commitment = await ZkUtils.calculateCommitment(move, saltValue);
    console.log('[commitMove] Commitment:', commitment);

    const client = this.createSigningClient(playerAddress, signer);

    // Convert commitment hex string to 32 bytes for the contract
    let commitmentHex = commitment;
    if (commitmentHex.startsWith('0x')) commitmentHex = commitmentHex.slice(2);
  ...
  /**
   * Reveal a move with ZK proof (Step 2 of ZK flow)
   */
  async revealMove(
    sessionId: number,
    playerAddress: string,
    ...
    console.log(`[revealMove] Player ${playerAddress} revealing move: ${move}. Using salt: ${saltValue}`);
    const commitment = await ZkUtils.calculateCommitment(move, saltValue);
    console.log(`[revealMove] Recalculated commitment for proof generation: ${commitment}`);

    // 2. Generate ZK Proof (UltraHonk)
    console.log('[revealMove] Generating ZK proof...');
    const proof = await ZkUtils.generateProof(move, saltValue, commitment);

    const client = this.createSigningClient(playerAddress, signer);

    // 3. Submit proof and move value to contract
    const proofBytes = (proof as any).proof ? (proof as any).proof : proof;
...
// Example: const zkGameTheoryService = new ZkGameTheoryService(contractId);
